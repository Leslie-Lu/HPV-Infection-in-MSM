...
%macro ORR(condition_list);
    ...
    %do %while (%scan(&condition_list, &i) ne );
        ...
        data _null_;
            set work.total;
            where genotype= &condition and outcome=1 and HIV_x= '1' and paramcd= 'ongo_fl';
            if count = 0 then call symput('count_outcome1_hiv1', 'yes');
            else call symput('count_outcome1_hiv1', 'no');
        run;
        data _null_;
            set work.total;
            where genotype= &condition and outcome=1 and HIV_x= '0' and paramcd= 'ongo_fl';
            if count = 0 then call symput('count_outcome1_hiv0', 'yes');
            else call symput('count_outcome1_hiv0', 'no');
        run;
        data _null_;
            set work.total;
            where genotype= &condition and outcome=0 and HIV_x= '1' and paramcd= 'ongo_fl';
        ...
        %if %upcase(%trim(&count_outcome1_hiv1)) eq YES and %upcase(%trim(&count_outcome0_hiv1)) eq YES %then %do;
            ...
            proc genmod data=work.total;
                where genotype= &condition and paramcd= 'ongo_fl';
            ...
            data diffs_&i;
                length genotype $35 paramcd $10 ORR $35 P $10;
                genotype = &condition;
                paramcd = 'ongo_fl';
                ORR= '/';
                P= '/';
                keep genotype paramcd ORR P;
            run;
            data LSMeans_&i;
                length genotype $35 paramcd $10 ORR $35 P $10;
                ...
                genotype = &condition;
                paramcd = 'ongo_fl';
                ORR= put(ExpEstimate*100, 5.2)||" ("||put(LowerExp*100, 5.2)||"-"||put(UpperExp*100, 5.2)||")";
                if compress(ORR) = compress('0.00 ( 0.00- . )') then ORR = '/';
                keep genotype paramcd HIV_x ORR;
                output;
        ...
        %else %if %upcase(%trim(&count_outcome1_hiv1)) eq NO and %upcase(%trim(&count_outcome1_hiv0)) eq NO %then %do;
            ...
            data diffs_&i;
                ...
                genotype = &condition;
                paramcd = 'ongo_fl';
                ORR= put(ExpEstimate, 5.2)||" ("||put(LowerExp, 5.2)||"-"||put(UpperExp, 5.2)||")";
                P= put(Probz, 5.3);
                keep genotype paramcd ORR P;
            run;
            data LSMeans_&i;
        ...
        %else %if %upcase(%trim(&count_outcome1_hiv1)) eq YES and %upcase(%trim(&count_outcome1_hiv0)) eq NO %then %do;
            ...
            data LSMeans_&i;
                ...
                paramcd = 'ongo_fl';
                if HIV_x=1 then ORR= '/';
                else ORR= put(ExpEstimate*100, 5.2)||" ("||put(LowerExp*100, 5.2)||"-"||put(UpperExp*100, 5.2)||")";
                keep genotype paramcd HIV_x ORR;
            run;
            proc transpose data=LSMeans_&i out=LSMeans_&i(drop=_NAME_) prefix=HIV_;
                by genotype paramcd;
...
%ORR(condition_list=%nrstr('HPV16' 'HPV18' 'HPV31' 'HPV33' 'HPV35' 'HPV39' 'HPV45' 'HPV51' 'HPV52' 'HPV56' 'HPV58' 'HPV59' 'HPV68'
...
data out_dat;
    retain genotype paramcd HIV_0 HIV_1 ORR P;
...
proc export data=out_dat
    outfile=".\output\CID_full_model_results\Table5_ORR.xlsx"
    dbms=excel replace;
run;
